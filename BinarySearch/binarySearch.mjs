function BinarySearch(arr, target, start, end) {
    ...
    if (arr[midIdx] === target) { // 타겟을 찾으면 타겟 리턴
        return midIdx;
    } else if (arr[midIdx] > target) { // 타겟이 중간 값보다 작으면 왼쪽 부분 배열 탐색
        return BinarySearch(arr, target, start, midIdx - 1);
    } else { // 타겟이 중간 값보다 크면 오른쪽 부분 배열 탐색
...
let target = BinarySearch(testArr, 47, 0, testArr.length - 1);
console.log(`target index:${target}, target value:${testArr[target]}`);
